...
public class BaiduLBSManager {
	...
	public LbsResultListener getLbsListener() {
		return lbsListener;
	}
	public void setLbsListener(LbsResultListener lbsListener) {
	...
	public void initLBS() {
		locationClient = new LocationClient(SXPApplication.getAppContext());
	...
	public class MyLocationListener implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (location.getLocType() == BDLocation.TypeGpsLocation) {// GPS定位结果
				...
				if(lbsListener!=null){
		        	lbsListener.error("服务端网络定位失败，可以反馈IMEI号和大体定位时间到loc-bugs@baidu.com，会有人追查原因");
		        }
				...
				if(lbsListener!=null){
		        	lbsListener.error("网络不同导致定位失败，请检查网络是否通畅");
		        }
			} else if (location.getLocType() == BDLocation.TypeCriteriaException) {
				sb.append("\ndescribe : ");
				sb.append("无法获取有效定位依据导致定位失败，一般是由于手机的原因，处于飞行模式下一般会造成这种结果，可以试着重启手机");
				if(lbsListener!=null){
		        	lbsListener.error("无法获取有效定位依据导致定位失败，一般是由于手机的原因，处于飞行模式下一般会造成这种结果，可以试着重启手机");
		        }
			}
			sb.append("\nlocationdescribe : ");
